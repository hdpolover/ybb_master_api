...
class Paper_abstracts extends RestController
{
    ...
    function list_get()
    {
        ...
        $paper_abstracts = $this->mCore->get_data('paper_abstracts', ['program_id' => $program_id, 'is_active' => 1])->result_array();
        if ($paper_abstracts) {
            $this->response([
                'status' => true,
                'data' => $paper_abstracts
    ...
    function participant_get()
    {
        ...
        $paper_abstracts = $this->mCore->get_data('paper_abstracts', ['visible_to <=' => $participant['general_status'], 'is_active' => 1])->result_array();
    ...
    public function upload_image($img_url, $id)
    {
        ...
        $data = $this->mCore->get_data('paper_abstracts', 'id = ' . $id)->row_array();
        if ($data['img_url'] != '') {
        ...
        if ($this->upload->do_upload($img_url)) {
            ...
            $sql = $this->mCore->save_data('paper_abstracts', ['img_url' => config_item('dir_upload') . 'announcements/' . $data['program_id'] . '/' . $fileName], true, array('id' => $id));
