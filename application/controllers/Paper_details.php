...
class Paper_details extends RestController
{
    ...
    function list_get()
    {
        ...
        $paper_details = $this->mCore->get_data('paper_details', ['program_id' => $program_id, 'is_active' => 1])->result_array();
        if ($paper_details) {
            $this->response([
                'status' => true,
                'data' => $paper_details
    ...
    function participant_get()
    {
        ...
        $paper_details = $this->mCore->get_data('paper_details', ['visible_to <=' => $participant['general_status'], 'is_active' => 1])->result_array();
    ...
    public function upload_image($img_url, $id)
    {
        ...
        $data = $this->mCore->get_data('paper_details', 'id = ' . $id)->row_array();
        if ($data['img_url'] != '') {
        ...
        if ($this->upload->do_upload($img_url)) {
            ...
            $sql = $this->mCore->save_data('paper_details', ['img_url' => config_item('dir_upload') . 'announcements/' . $data['program_id'] . '/' . $fileName], true, array('id' => $id));
